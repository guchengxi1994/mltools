        ...
        if os.path.isfile(txtPath):
            ...
            if len(contents) > 0:
                for c in contents:
                    obj = dict()
                    tmp = c.split(" ")
                    clas, x, y, w, h = int(tmp[0]), tmp[1], tmp[2], tmp[3], tmp[4]

                    bbox = convert(imgShape, float(x), float(y), float(w), float(h))
                    # print(bbox)
            ...
            logger.info("Done! See {} .".format(parent_path + os.sep + "_xmls_"))
